...
/**
 * An instance of this class is used to generate a stream of pseudorandom numbers, similarly to
 * {@link java.util.Random}. <p> The main differences are: <ul> <li><a
 * href="http://www.pcg-random.org/">PCG algorithm</a> is used; in particular, this is a port of the
 * <a href="https://github.com/imneme/pcg-c-basic/">minimal C implementation</a>.</li> <li>Instances
 * of Pcg32 are not thread-safe and so it doesn't obey {@link java.util.Random} contract.</li>
 * </ul>
 */
public class Pcg32
{
  ...
  /**
   * Creates a new random number generator using a {@code long} seed and a {@code long} stream
   * number. <p> The invocation {@code new Pcg32(initState, initSeq)} is equivalent to:
   * <pre> {@code
   * Pcg32 rnd = new Pcg32();
  ...
  /**
   * Creates a new random number generator using current time (returned by {@link
   * System#nanoTime()}) as the seed and a unique stream number.
   *
   * @see #seed()
   */
  public Pcg32()
  ...
  /**
   * Returns the next pseudorandom, approximately uniformly distributed {@code float} value
  ...
  /**
   * Returns the next pseudorandom, approximately uniformly distributed {@code double} value
   * between 0.0 (inclusive) and 1.0 (exclusive).
   *
